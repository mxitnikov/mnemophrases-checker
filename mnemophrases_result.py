def main(f):
    ...
    with open(f, 'r') as f:
        while True:
            ...
            if not line:
                print(f'\n')
                print(f'Обработка лога завершена. Собрано {count} строк.')
                print(f'Ваши мнемофразы собраны построчно в файле result.txt')
                print(f'Этот файл появится в директории скрипта!')
                print(f'Скрипт сделал - t.me/mxitnikov')
                break

            am = line.split(' ')
            if len([i for i in am if i in words]) == 11:
                count += 1
                print(f'[{count}] Добавлена строка из 12 мнемофраз')
                a.writelines(str(" ".join(am)))

            elif len([i for i in am if i in words]) == 23:
                count += 1
                print(f'[{count}] Добавлена строка из 24 мнемофраз')
                a.writelines(str(" ".join(am)))

            elif len([i[:12] for i in am if i in words]) == 12:
                count += 1
                print(f'[{count}] Добавлена строка из 12 мнемофраз')
                a.writelines(str(" ".join([i[:12] for i in am if i in words]) + '\n'))

            elif len([i[:24] for i in am if i in words]) == 24:
                count += 1
                print(f'[{count}] Добавлена строка из 24 мнемофраз')
                a.writelines(str(" ".join([i[:24] for i in am if i in words]) + '\n'))


if __name__ == '__main__':
    main(input('>>> Введите путь до файла: '))
